...
#for  Cycling
y_n = read_tsv("ranks_EN_upper_w20_arch_yellow_vs_nocolor_recorrect_umi_F_sct_cca_wilcox_lfc.rnk")
y_n.gsea <- y_n$rank
names(y_n.gsea) <- as.character(y_n$GeneName)
y_n.gsea <- sort(y_n.gsea, decreasing = T)

#for  IPC
g_n = read_tsv("ranks_EN_upper_w20_arch_green_vs_nocolor_recorrect_umi_F_sct_cca_wilcox_lfc.rnk")
g_n.gsea <- g_n$rank
names(g_n.gsea) <- as.character(g_n$GeneName)
g_n.gsea <- sort(g_n.gsea, decreasing = T)

#for  EN_upper_w10
r_n = read_tsv("ranks_EN_upper_w20_arch_red_vs_nocolor_recorrect_umi_F_sct_cca_wilcox_lfc.rnk")
r_n.gsea <- r_n$rank
names(r_n.gsea) <- as.character(r_n$GeneName)
r_n.gsea <- sort(r_n.gsea, decreasing = T)

#for  EN_upper_w10
y_r = read_tsv("ranks_EN_upper_w20_arch_yellow_vs_red_recorrect_umi_F_sct_cca_wilcox_lfc.rnk")
y_r.gsea <- y_r$rank
names(y_r.gsea) <- as.character(y_r$GeneName)
y_r.gsea <- sort(y_r.gsea, decreasing = T)

#for  EN_upper_w10
g_r = read_tsv("ranks_EN_upper_w20_arch_green_vs_red_recorrect_umi_F_sct_cca_wilcox_lfc.rnk")
g_r.gsea <- g_r$rank
names(g_r.gsea) <- as.character(g_r$GeneName)
g_r.gsea <- sort(g_r.gsea, decreasing = T)

#for  EN_upper_w10
y_g = read_tsv("ranks_EN_upper_w20_arch_yellow_vs_green_recorrect_umi_F_sct_cca_wilcox_lfc.rnk")
y_g.gsea <- y_g$rank
...
test.out<- compareCluster(geneCluster=inputList, fun="GSEA", pvalueCutoff=0.05,
                          ...
                          seed = T)
saveRDS(test.out, "w20_arch_EN_upper_6pairwise_comparecluster_10_500_0.05_3.10.23.rds")
test.out <- readRDS("w10_EN_d_u_IN_neuron_cluster_clay_vs_chap_comparecluster_10_500_0.05_2.19.23.rds")
